...
BOOST_AUTO_TEST_CASE(expr_print) {
  ...
  c = mk_list(
      {mk_list({mk_atom("hello"),
                mk_list({mk_atom("hello"), mk_atom("there"), mk_atom("jim")}),
                mk_atom("there"), mk_atom("jim")}),
       mk_atom("hello"), mk_atom("there"), mk_atom("jim"),
       mk_list({mk_atom("hello"), mk_atom("there"), mk_atom("jim")})});
  BOOST_CHECK_EQUAL(to_string(c), "((hello (hello there jim) there jim) hello "
                                  "there jim (hello there jim))");
  (ss = stringstream()) << c;
  BOOST_CHECK_EQUAL(ss.str(), "((hello (hello there jim) there jim) hello "
                              "there jim (hello there jim))");
...
BOOST_AUTO_TEST_CASE(expr_eq_test) {
  ...
  BOOST_CHECK(expr_equal(mk_list(mk_atom("a")),
                         mk_list({mk_atom("a"), mk_atom("b")})) == sF);
  BOOST_CHECK(expr_equal(mk_list({mk_atom("a"), mk_atom("b")}),
                         mk_list(mk_atom("a"))) == sF);

  BOOST_CHECK(
      expr_equal(mk_list({mk_atom("a"), mk_list({mk_atom("a"), mk_atom("b")})}),
                 mk_list({mk_atom("a"),
                          mk_list({mk_atom("a"), mk_atom("b")})})) == sT);

  BOOST_CHECK(
      expr_equal(mk_list({mk_atom("a"), mk_list({mk_atom("a"), mk_atom("b")})}),
                 mk_list({mk_atom("a"),
                          mk_list({mk_atom("a"), mk_atom("c")})})) == sF);
}
