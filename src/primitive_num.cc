...
bool is_all_integer(const Expr &a) {
    Expr args{a};
    while (!is_false(args)) {
...
{
    return [=](const Expr &args) -> Expr { return mk_float(f(as_float(args->car))); };
}
